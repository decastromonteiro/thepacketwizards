...
def home_page(request):
    ...
    qs = (
        ...
        .filter(published=True, featured=True)[:3]
    )
...
def blog_index(request):
    ...
    qs = (
        BlogPost.objects.all()
        .order_by("-publish_date")
        .filter(published=True, publish_date__lte=now)
    )
...
def blog_category(request, category):
    ...
    try:
        qs = (
            BlogPost.objects.filter(category__slug__exact=category)
            .order_by("-publish_date")
            .filter(published=True, publish_date__lte=now)
        )
        if qs:
            context = {
                "title": "Category: {}".format(category),
                "blog_list": qs,
...
def blog_series(request, series):
    ...
    try:
        qs = BlogPost.objects.filter(
            series__slug__exact=series, published=True, publish_date__lte=now
        ).order_by("-publish_date")
        if qs:
            context = {
                "title": "Series: {}".format(series),
                "blog_list": qs,
